...
public abstract class AbstractAmqpMessageProducer
{
     ...
     */
    private static final ILogger LOGGER = RabbitMQLoggingManager.getLogger(AbstractAmqpMessageProducer.class);
     ...
     * @since 1.0
     */
    public AbstractAmqpMessageProducer(final RabbitTemplate rabbitTemplate, final String hostname)
    {
        super();
     ...
     * @since 1.0
     */
    protected void publishMessage(final String correlationId, final String replyTo, final String exchangeName, final String routingKey,
            final Object message) throws AmqpException
    {
        final MessageProperties messageProperties = MessagePropertiesHelper
                .makeMessageProperties(this.getCalendar().getTime(), correlationId, replyTo);

        final PropertiesPostProcessor messagePostProcessor = new PropertiesPostProcessor(messageProperties);
        ...
        if (exchange == null)
        {
            ...
            final String logMessage = LOGGER.error(RabbitMQMessageCode.NO_EXCHANGE_FOUND_E.getMessageCode(), logParams);
        ...
        this.getRabbitTemplate().convertAndSend(exchange.getName(), routingKey, message, messagePostProcessor);
    }
}
